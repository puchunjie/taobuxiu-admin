        ...
        <div class="want-content clearfix">
            <div class="item-list">
                <div class="item-content">
                    ...
                    :class="activeItemInde === index ? 'active' : ''"
            ...
            <div class="item-detail">
                <w-item :itemData="activeItem"></w-item>
            </div>
        </div>
    </div>
</template>
...
<script>
    import { myIronBuys , sMyBuyDetail } from '@/utils/http'
    ...
    export default {
        ...
        methods: {
            init(){
                ...
                this.tabList.map(el => {
                    if(el.status === -1){
                        ...
                        this.getBuyData(el,0)
                    }
                })
            },
            //获取列表数据
            getBuyData(item,pageCount){
                this.itemListLoad = false;
                this.$http.get(myIronBuys,{
                    params:{
                    ...
                    this.tabList[item.sort].count = data.maxCount;
                })
            },
            //获取资源详情
            getDetial(id){
            ...
            getPageDate(page){
                ...
                this.getBuyData(this.tabList[this.activeTab]);
            },
            itemStatus(status){
                let ststusStr = '';
                switch (status) {
            ...
            changeDate(item,index){
                ...
                this.currentPage = 0;
                this.activeItemInde = 0;
                this.getBuyData(item);
            },
            //详情数据切换
            switchItme(id,index){
                if(this.activeItemInde != index){
                    this.getDetial(id);
                    this.activeItemInde = index;
                }
            }
        }
    }
